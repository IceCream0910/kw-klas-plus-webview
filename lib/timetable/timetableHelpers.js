...
export const getNativeTimetableColor = (color) => {
    if (!color || typeof color !== 'object' || typeof color.palette !== 'string' || color.palette.length === 0 ||
        !Number.isInteger(color.slot) || color.slot < 0) {
        return null;
    }

    const themes = ['light', 'dark'];
    if (!themes.every(theme =>
        color[theme] &&
        typeof color[theme].background === 'string' &&
        typeof color[theme].text === 'string' &&
        HEX_COLOR.test(color[theme].background) &&
        HEX_COLOR.test(color[theme].text)
    )) {
        return null;
    }
