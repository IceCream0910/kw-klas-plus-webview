...
export const formatCalendarEvents = (events) => {
    return events.map(event => {
        ...
        let startedM = moment(event.started, "YYYYMMDDHHmm");
        const endedM = moment(event.ended, "YYYYMMDDHHmm");

        // 학교 측 26년 1학기 수강신청 입력 오류 대응) 기간이 1년 이상이면 오류로 간주하고 시작일 올해로 수정
        if (event.typeNm === "학사일정" && endedM.diff(startedM, 'days') > 365) {
            startedM.year(endedM.year());
            if (startedM.isAfter(endedM)) {
                startedM.subtract(1, 'year');
            }
        }
